...
class TaskList(APIView):
    ...
    def post(self, request):  # Create new task
        ...
        serializer = TaskSerializer(
            data=request.data
        )  # take data convert into python understandable to JSON
        if serializer.is_valid():
            serializer.save(user=request.user)  # save data  instance
            return Response(serializer.data)
        return Response(
            status=HTTP_400_BAD_REQUEST
        )  # if data is not in valid format return 400 error


class IsOwner(
    permissions.BasePermission
):  # custom permission to access particular tasks
    def has_object_permission(self, request, view, obj):
        if request.method in permissions.SAFE_METHODS:
            return True
        return (
            obj.user == request.user
        )  # check for is given user is owner of object or not
